...
bool List:: PriorElem(Coordinate* currentElem, Coordinate* preElem)  //获取指定元素的前驱元素
{
...
void List::ListTraverse()  //遍历
{
	for (int i = 0; i < m_iLength; i++)
	{
		cout << m_pList[i] << endl;  //用到运算符重载
	}
}


bool List::ListInsert(int i,Coordinate* e)  //在第i个位置插入元素e
{
	if (i < 0 || i > m_iLength)
	{
		return false;
...
bool List::ListDelete(int i, Coordinate* e)  //删除第i个位置的元素
{
	if (i < 0 || i >= m_iLength)
	{
		return false;
